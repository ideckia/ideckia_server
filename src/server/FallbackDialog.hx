...
class FallbackDialog implements Dialog {
	...
	public function setOptions(options:Any) {
		trace('Called dialog.$setOptions function.');
		trace(FALLBACK_MESSAGE);
	}
	...
	public function question(title:String, text:String):Promise<Bool> {
		trace('Called dialog.$question function.');
		trace(FALLBACK_MESSAGE);
		return Promise.resolve(true);
	}

	public function selectFile(title:String, isDirectory:Bool = false, multiple:Bool = false, ?fileFilter:FileFilter):Promise<Array<String>> {
		trace('Called dialog.$selectFile function.');
		trace(FALLBACK_MESSAGE);
		return Promise.resolve(['file0path', 'file1path']);
	}

	public function saveFile(title:String, ?saveName:String, ?fileFilter:FileFilter):Promise<String> {
		trace('Called dialog.$saveFile function.');
		trace(FALLBACK_MESSAGE);
		return Promise.resolve('filepath');
	}

	public function entry(title:String, text:String, placeholder:String = ''):Promise<String> {
		trace('Called dialog.$entry function.');
		trace(FALLBACK_MESSAGE);
		return Promise.resolve(placeholder);
	}

	public function password(title:String, text:String, showUsername:Bool = false):Promise<Array<String>> {
	...
	public function color(title:String, ?initialColor:String, palette:Bool = false):Promise<Color> {
	...
	public function calendar(title:String, text:String, ?year:UInt, ?month:UInt, ?day:UInt, ?dateFormat:String):Promise<String> {
		trace('Called dialog.$calendar function.');
		trace(FALLBACK_MESSAGE);
		return Promise.resolve(Date.now().toString());
	}

	public function list(title:String, text:String, columnHeader:String, values:Array<String>, multiple:Bool = false):Promise<Array<String>> {
